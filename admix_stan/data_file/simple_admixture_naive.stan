...
model {
    ...
    for (i in 1:N_obs) {
      ...
      if (group[i][1]==0 && group[i][2]==0){
        number[i]~poisson((int_p_L_divide_l(N[1],0,T2,u[i],v[i]) + int_p_L_divide_l(N[4],T2,100000000,u[i],v[i]))*length);
      }
      // This is the sharing number between population A and Admix
      else if (group[i][1]==0 && group[i][2]==1){
        number[i]~poisson((fraction * int_p_L_divide_l(N[1],T1,T2,u[i],v[i]) + int_p_L_divide_l(N[4],T2,100000000,u[i],v[i]))*length);
      }
      // This is the sharing number between population A and C
      else if (group[i][1]==0 && group[i][2]==2){
        number[i]~poisson(int_p_L_divide_l(N[4],T2,100000000,u[i],v[i])*length);
      }
      // This is the sharing number between population Admix and Admix
      else if (group[i][1]==1 && group[i][2]==1){
        number[i]~poisson((int_p_L_divide_l(N[2],0,T1,u[i],v[i]) + pow(fraction,2) * int_p_L_divide_l(N[1],T1,T2,u[i],v[i]) + pow(1-fraction,2)*int_p_L_divide_l(N[3],T1,T2,u[i],v[i]) + int_p_L_divide_l(N[4],T2,100000000,u[i],v[i]))*length);
      }
      // This is the sharing number between population Admix and C
      else if (group[i][1]==1 && group[i][2]==2){
        number[i]~poisson(((1-fraction) * int_p_L_divide_l(N[3],T1,T2,u[i],v[i]) + int_p_L_divide_l(N[4],T2,100000000,u[i],v[i]))*length);
      }
      // This is the sharing number between population C and C
      else{
        number[i]~poisson((int_p_L_divide_l(N[3],0,T2,u[i],v[i]) + int_p_L_divide_l(N[4],T2,100000000,u[i],v[i]))*length);
